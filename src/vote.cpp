    ...
    BOOST_FOREACH(const CParkRateVote& parkRateVote, vParkRateResult)
    {
        ...
        for (unsigned int i = 0; i < vSortedParkRate.size(); i++)
        {
            ...
            if (nDuration < parkRate.GetDuration())
            {
                ...
                int ratio = (int)(parkRate.nRate - prevParkRate.nRate) / (parkRate.GetDuration() - prevParkRate.GetDuration());
                int rate = prevParkRate.nRate + (nDuration - prevParkRate.GetDuration()) * ratio;
