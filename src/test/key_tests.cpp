...
BOOST_AUTO_TEST_CASE(key_test2)
{
    ...
    bool fCompressed;
    BOOST_CHECK_EQUAL(CBitcoinSecret(bsecret1.GetSecret(fCompressed), fCompressed, 'S').ToString(), strSSecret1);
    BOOST_CHECK_EQUAL(CBitcoinSecret(bsecret1.GetSecret(fCompressed), fCompressed, 'B').ToString(), strBSecret1);
}
