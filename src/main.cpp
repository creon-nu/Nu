...
// nu: Maximum value of the outputs generated by Proof of Work
#define INITIAL_OUTPUT_VALUE (100000 * COIN)
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (pblock->IsProofOfWork())
    {
        ...
        if (GetBoolArg("-splitshareoutputs", true) && nReward >= INITIAL_OUTPUT_VALUE * 2)
        {
            // nu: split output of generated shares
            int nOutputs = nReward / INITIAL_OUTPUT_VALUE;
            int64 nRemainingAmount = nReward;

            for (int i = 0; i < nOutputs - 1; i++)
            {
                int64 nAmount = INITIAL_OUTPUT_VALUE;
